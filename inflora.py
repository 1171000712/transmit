...
class InfLoRA(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        for name, param in self._network.named_parameters():
            if param.requires_grad:
                enabled.add(name)
                if "A" in name and  self._cur_task > 0:
                    tmp=param.data-torch.mm(param.data,self.feature_mat[kk//2].to(4))
                    kk+=1
        ...
        if self._cur_task == 0:
            ...
            self.train_function(train_loader, test_loader, optimizer, scheduler,enabled)
        else:
            if self.optim == 'sgd':
        ...
        kk=0
        for name, param in self._network.named_parameters():
            if name in enabled and 'lora_A' in name:
            # Update LoRA A from lora_a0
                if self._cur_task > 0:
                    new_a =  param.data - torch.mm(param.data,self.feature_mat[kk//2].to(self._device))
                    param.data.copy_(new_a.data)
                    kk+=1
                ...
                # Calculate generalized inverse of new A
                #lora_b_name = name.replace('lora_A', 'lora_B')
        
    
                lora_b = self._network.state_dict()[lora_b_name]
                param_b = lora_b.clone()
                                        
                # Compute LoRA B using generalized inverse
                generalized_inverse_a = torch.linalg.pinv(param.data)
            
                param_b.data = torch.mm(torch.mm(param_b.data,param.data),generalized_inverse_a)
                #print(param_b.data-lora_b.data)
                self._network.state_dict()[lora_b_name].data.copy_(param_b.data)
        ...
        with torch.no_grad():
            ...
            mat_list = []
            kk=0
            for _,module in enumerate(self._network.modules()):
                if isinstance(module, Attention_LoRA):
                    #mat_list.append(deepcopy(module.cur_matrix))
                    mat_list.append(deepcopy(self.tmp_matrixs[kk]))
                    kk+=1
    ...
    def train_function(self, train_loader, test_loader, optimizer, scheduler,enabled):
        import random
        kk=0
        for name, param in self._network.named_parameters():
            if name in enabled and 'lora_A' in name:
                kk+=1
        self.tmp_matrixs = [0]*(kk//2)
        self.tmp_n = [0]*(kk//2)
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                lora_a = {name: param.data.clone() for name, param in self._network.named_parameters() if name in enabled and 'lora_A' in name}
                ...
                lora_gradients = {name: param.grad.clone() for name, param in self._network.named_parameters() if name in enabled and 'lora_A' in name}
                ...
                for name, param in self._network.named_parameters():
                    if name in enabled and 'lora_A' in name:
                        if epoch==self.run_epoch-1 :
                            self.tmp_matrixs[kk//2]=(self.tmp_matrixs[kk//2]*self.tmp_n[kk//2]+torch.mm(lora_gradients[name].T, lora_gradients[name]).cpu()) / (self.tmp_n[kk//2] + len(lora_gradients[name]))
                            self.tmp_n[kk//2] += len(lora_gradients[name])
                            
                        # Update LoRA A 
                        if self._cur_task == 0 or random.random()<1:
                            new_a = lora_a[name] - step_size * lora_gradients[name]
                            kk+=1
                        else:
                            new_a = lora_a[name] - step_size * (lora_gradients[name] - torch.mm(lora_gradients[name],self.feature_mat[kk//2].to(self._device)))
                            
                            #print(torch.mm(new_a,self.feature_mat[kk//2].to(self._device)))

                            kk+=1
                        
                        
                        #print(name,param.data-new_a.data)  
                        param.data.copy_(new_a.data)
                        
                        # Calculate generalized inverse of new A
                        lora_b_name = name.replace('lora_A', 'lora_B')
                        
                        if lora_b_name in enabled:
                            lora_b = self._network.state_dict()[lora_b_name]
                            param_b = lora_b.clone()
                            ...
                            param_b.data = torch.mm(torch.mm(param_b.data,param.data),generalized_inverse_a)
                            #print(lora_b_name,self._network.state_dict()[lora_b_name].data-param_b.data)
                            self._network.state_dict()[lora_b_name].data.copy_(param_b.data)
